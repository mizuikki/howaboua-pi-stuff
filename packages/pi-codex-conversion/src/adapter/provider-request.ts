...
import { isEffectiveOpenAICodexContext, shouldUseCodexRuntimeFeatures } from "./activation/activation.ts";
...
export async function rewriteCodexProviderRequest(payload: unknown, ctx: ExtensionContext, state: AdapterState): Promise<unknown | undefined> {
	if (!shouldUseCodexRuntimeFeatures(ctx, state.config) || (!isEffectiveOpenAICodexContext(ctx, state.config) && !isResponsesContext(ctx))) {
		return undefined;
	}
